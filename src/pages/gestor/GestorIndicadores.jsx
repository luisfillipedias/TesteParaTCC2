...
export default function GestorIndicadores() {
  ...
  useEffect(() => {
    ...
    import('https://cdn.jsdelivr.net/npm/chart.js@4.4.1/dist/chart.umd.min.js').catch(() => {}).then(() => {
      ...
      if (barRef.current) c1 = new window.Chart(barRef.current.getContext('2d'), { type:'bar', data:{ labels: stats.espLabels || [], datasets:[{label:'Atendimentos',data: stats.espData || [],backgroundColor:['#1351B4','#0C326F','#155BCB','#2670E8','#071D41','#268744'],borderRadius:4}]}, options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:'#CCCCCC'}},x:{grid:{display:false}}}}});
      if (lineRef.current) c2 = new window.Chart(lineRef.current.getContext('2d'), { type:'line', data:{ labels: stats.tempoLabels || [], datasets:[{label:'Dias',data: stats.tempoData || [],borderColor:'#1351B4',backgroundColor:'rgba(19,81,180,0.08)',fill:true,tension:0.4,pointRadius:5,pointBackgroundColor:'#1351B4'}]}, options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:'#CCCCCC'}},x:{grid:{display:false}}}}});
    });
